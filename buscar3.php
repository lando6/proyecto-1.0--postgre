<?php
    ...
    include 'conex.php';
    $salida = "";
    $query = "SELECT * FROM productos_o WHERE estatus=1 ORDER BY nombre";
    if (isset($_POST['consulta'])) 
    {
        $q = pg_escape_string($conex,$_POST['consulta']);
        $query = "SELECT * FROM productos_o WHERE nombre LIKE '%$q%' AND estatus=1 ORDER BY nombre";
    }
        ...
        elseif($_SESSION['tipo_de_usuario'] == 'usuario' or $_SESSION["tipo_de_usuario"] == "super-usuario")
        {
            ...
            while ($fila = pg_fetch_assoc($resultado)) 
            {
                ...
                if(isset($_SESSION["cart"]))
                {
                    ...
                    {
                        if($c["product_id"] == $fila["id"] && $c["type"] == "o")
                        {
                            $found=true;
                            break;
                        }
